...
| PR-DESKTOP-01 desktop practice-only workflow       | RT-04, UI-21-24, UI-27, UI-30-31                             | Passed       |
| PR-MOTION-01 reduced motion                        | R-DEGRADED, UI-19, UI-32                                     | Passed       |
| `scrollRace.test.ts`                               | RT-02; UI-02, UI-18, UI-20-22, UI-31                         | 33 passed    |
| `localRaceStore.test.ts`                           | R-RETURN, R-DEGRADED; UI-02-04, UI-07-12, UI-23-26           | 7 passed     |
| `board.test.ts`                                    | RT-03-04; UI-24, UI-31, UI-33                                | 21 passed    |
| `globalBoard.test.ts`                              | UI-11, UI-33                                                 | 3 passed     |
| `device.test.ts`                                   | RT-04, UI-24, UI-31                                          | 2 passed     |
...
- 13/13 browser scenarios passed.
- 4/4 event workflows completed with eligible submissions, plus a same-player
  rerun that retained exactly one local leaderboard row.
- 66/66 unit/integration tests passed.
- No page errors or console errors occurred in the clean browser run.
- Local boards stayed at 10 entries with one fastest result per normalized
  player name, and world fixtures stayed at the 100-entry storage cap.
